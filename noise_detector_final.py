...
#         peaks, _ = find_peaks(column_averages, height=height_threshold_peaks)
#         # Detect valleys (negative peaks)
#         if checkbox_valleys_state:
#             valleys, _ = find_peaks(-column_averages, height = height_threshold_valley) 
...
#     # Now, compute mean of column_averages for fragmented peaks only from the split where they appeared
#     fragmented_values = []
#     mean_fragmented_intensity = dict()
#     for peak in temporal_fragmented_peaks:
#         for split_idx, split_peaks in enumerate(all_peaks_of_frame):
#             if peak in split_peaks:
#                 fragmented_values.append(all_column_averages[split_idx][peak])

#         mean_fragmented_intensity[peak] = np.mean(fragmented_values) if fragmented_values else 0
#         fragmented_values.clear()  # Clear for the next peak

#     return temporal_noisy_peaks, temporal_fragmented_peaks, mean_fragmented_intensity
...
#             column_averages = frame_clipped.mean(axis=0)
#             mean_intensity = np.mean(column_averages)
            
#             # Process peaks for this frame
#             temporal_noisy_peaks, temporal_fragmented_peaks, mean_fragmented_intensity = process_frame_peaks(frame_clipped, checkbox_valleys_state)
...
#                         y_value = abs(column_averages[peak] - mean_intensity)  # Ensure y_value is non-negative
...
#                         y_value = abs(mean_fragmented_intensity[peak] - mean_intensity)  # Ensure y_value is non-negative
